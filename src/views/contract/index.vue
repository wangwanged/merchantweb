      ...
      <div class="fl">
        ...
        <el-col :span="1.5">
          <el-button
            type="primary"
            size="mini"
            @click='$store.state.Soso.contract.dialogRenew=true'
            :disabled="single"
            v-hasPermi="['contract:contractManager:add']"
          >续签
          </el-button
...
<!--          handleRenew-->
    ...
    <!-- 续签弹框 -->
    <el-dialog title="续签合同" :visible.sync="$store.state.Soso.contract.dialogRenew" width="700px">
      <el-form label-position="left" label-width="110px">
        ...
        <el-form-item  required label="签约产品">
          <el-select v-model="form.produce" disabled>
            <el-option
...
export default {
  ...
  computed: {
    sum() {
      let sum = 0
      console.log("this.fee.jingyingManagerFee.detail", this.fee.jingyingManagerFee.detail)
      this.fee.jingyingManagerFee.detail.forEach((item, index) => {
  ...
  methods: {
    ...
    submitRenew() {
      ...
      this.fee
      // this.form.fee.jingyingManagerFee.detail.
      this.form.guarantee = this.form.fee.lvyueFee
